 ...
 */
@Service
public class DefaultAATService implements AATService
{

    private static final Logger LOG = LoggerFactory.getLogger(DefaultAATService.class);

    private final CarRepository carRepository;

    public DefaultAATService(final CarRepository carRepository)
    {
    ...
    @Override
    @Transactional
    public void updateStatus(long carId, String carStatus) throws EntityNotFoundException
    {
        CarDO carDO = findCarChecked(carId);
        carDO.setCarStatus(CarStatus.valueOf(carStatus));
    }
    ...
    private CarDO findCarChecked(Long carId) throws EntityNotFoundException {
        return carRepository.findById(carId).orElseThrow(() -> new EntityNotFoundException("Could not find entity with id: " + carId));
    }
